...
function generateFlags(headers: Record<string, unknown>, userAgent: string): Device {
  let mobile = false
...
export function useDevice(params?: { refreshOnResize?: boolean }) {
    type Keys = keyof Device
    const userAgent = navigator.userAgent
    const flags = reactive(generateFlags({}, userAgent))

    if (params?.refreshOnResize) {
      useEventListener('resize', () => {
        setTimeout(() => {
          const newFlags = generateFlags({}, navigator.userAgent)
          for (const key in newFlags) (flags as any)[key] = newFlags[key as Keys]
        }, 50)
      })
    }
