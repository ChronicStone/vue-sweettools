...
export function propertyBuilderFilter(
      ...
      return {
        ...
        transformFilterValue: (rawValue: Array<FilterBuilderRawValue>) =>
          rawValue.map(({ propertyName, matchMode, ...values }) => ({
            [propertyName]:
              "value" in values ? values.value : values[`value-${matchMode}`],
          })),
      };
    },
    collapsible: false,
    ...
    fields: [
      {
        ...
        label: "Property",
        type: "select",
        options: params.properties.map((p) => ({
          label: p.label,
          value: p.key,
        })),
      },
      {
        ...
        options: (deps) => {
          const property = params.properties.find(
            (p) => p.key === deps.propertyName,
          );
          if (!property) return matchModes;

          return matchModes.filter(
            (m) => property.matchModes?.includes(m.value) ?? true,
          );
        },
      },
      ...params.properties
          ...
          return [
                      ...
                      (field) =>
                        ({
                          ...
                          ...(field?.matchMode && {
                            dependencies: [
                              ...(field?.dependencies ?? []),
                              ["$parent:1.matchMode", "__$matchMode"],
                            ],
                            condition: (deps, api) =>
                              (field?.condition?.(deps, api) ?? true) &&
                              deps.__$matchMode === field.matchMode,
                          }),
                        }) as FormField,
                    ),
            ...
            ...(typeof p.field === "function"
              ? matchModes.map((m) => ({
                  ...
                  dependencies: [
                    ["$parent.propertyName", "propertyName"],
                    ["$parent.matchMode", "matchMode"],
                  ],
                  condition: (deps: Dependencies) => {
                    return (
                      deps.propertyName === p.key &&
                      !!deps?.matchMode &&
                      deps.matchMode === m.value
                    );
                  },
                }))
              : ({
                  ...
                  condition: (deps: Dependencies) =>
                    deps.propertyName === p.key && !!deps?.matchMode,
                } as any)),
          ];
        })
        .flat(),
