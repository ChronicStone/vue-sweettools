import type { DataSource, FetchParams } from '../types/shared'
  ...
  try {
    ...
    if (sortKey) {
      output = output.sort((a, b) => {
        const aValue = getObjectProperty(sortKey, [], a) as any
        const bValue = getObjectProperty(sortKey, [], b) as any
        if (aValue === bValue)
          return 0
        if (aValue > bValue)
    ...
    if (query && Object.keys(query).length > 0) {
      for (const key in query) {
        ...
        output = output.filter((item) => {
          const value: any = getObjectProperty(key, [], item)
